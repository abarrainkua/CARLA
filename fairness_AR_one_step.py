...
file_path = f"{dataset_path}_train.csv"
results_file_path = "training_metrics_log.csv"


my_trained_model, train_metrics = train_social_burden(dataset='adult', file_path=file_path, sens_attr='race', lr=0.001, epochs=total_epoch, batch_size=256, 
                                       hidden_sizes=[128, 128], activation_name="relu", verbose=True, pretrain_epochs=pretrain_epoch, 
                                       recourse_method="gs", weighing_strategy="individual", recourse_hyperparam={}, 
                                       results_file=results_file_path, random_state=42)
...
plt.plot(train_metrics["epoch"], train_metrics["fpr_group_1.0"], label="FPR Group 1", linestyle="--", color="orange")
plt.axvline(pretrain_epoch, color='gray', linestyle=':', linewidth=2, label="Pretraining End")
...
plt.savefig("minmaxburden_training_metrics_plot.png", dpi=300, bbox_inches='tight')
# plt.show()
